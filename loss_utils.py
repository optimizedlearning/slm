# Various helper functions for computing losses
...
def get_loss_data(
    model: Union[Callable, torch.nn.Module], batch: Any, ignore_index: int = -100
) -> Dict:
    # computes a dict containing:
    # loss, logits, accuracy and possibly bits per byte
...
def get_output_from_batch(
    ...
    # computes the logits (model output)
...
def get_only_loss_from_logits(
    logits: torch.Tensor, batch: Any, ignore_index: int = -100
) -> Dict:
    # returns only the loss tensor (not a loss_data dict).
...
def get_loss_data_from_logits(
    ...
    if "tokenized_bytes" in batch:
        ...
        result["bits_per_byte"] = (
            loss * torch.sum(targets != ignore_index)
        ) / total_bytes
